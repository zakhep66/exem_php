...
<body>
	...
	if (!isset($_SESSION[qn])) {
		...
		if ($submit == 'Next Question' && isset($ans)) {
			...
			echo "<h1 class=head1>Результат</h1>";
			$_SESSION[qn] = $_SESSION[qn] + 1;
			echo "<Table align=center><tr class=tot><td>Вопрос<td> $_SESSION[qn]";
			echo "<tr class=tans><td>Правильный ответ<td>" . $_SESSION[trueans];
			$w = $_SESSION[qn] - $_SESSION[trueans];
			echo "<tr class=fans><td>Неправильный ответ<td> " . $w;
			echo "</table>";
	...
	if ($_SESSION[qn] > mysqli_num_rows($rs) - 1) {
		unset($_SESSION[qn]);
		echo "<h1 class=head1>Что-то пошло не так</h1>";
		session_destroy();
		echo "Please <a href=index.php>Начать заново</a>";
